...
def R(zeta,rho,k): #interest rate
...
#Using k's from above approximation along Phis
k2_tmrw[0]=k_tmrw[0]
k2_tmrw[0]=k
i=1
...
while i<T:
    ...
    S=np.random.choice(s[:,j], size=T)
    lnkk=np.log(S)+np.log(1-tau)+np.log(1-alpha)+np.log(zeta[j])-np.log(1+lam)-np.log(1+g)+alpha*np.log(kt_tmrw[i])
    kk=np.exp(lnkk)
    k2_tmrw[i]=kk 
    k=kk
    cons_1[i]=((1-tau)*wt(k,zeta[j]))*(1-S) #(1-S)*(1-tau)*(1-alpha)*zeta[j]*kt_tmrw[i]**alpha
    cons_2[i]=(1-tau)*wt(k,zeta[j])*S*R(zeta[j],rho_T[i],k)+lam*eta_T[i]*wt(kt_tmrw[ii],zeta[jj])*(1-tau)+(1-lam)*tau*wt(k,zeta[i])*(1+lam)/(1-lam) #beta*cons_1[i]*(1+alpha*kt_tmrw[i]**(alpha-1)*zeta[j]*rho_[i])
    i=i+1 
